...
(function($) {
  ...
  $.GollumPreview = function(IncomingOptions) {
    ...
    var Fragments = {
      ...
      renderRemote: function(fragments, callback) {
        if (ActiveOptions.FragmentsUrl != null) {
          $.ajax({
            url: ActiveOptions.BaseUrl + ActiveOptions.FragmentsUrl,
            type: "POST",
            accepts: "json",
            data: {
              fragments: fragments,
    ...
    var Renderer = {
      ...
      postProcess: function(rendered){
        ...
        // TODO: looks like this filters ~~strikethrough~~ in *all* markups. Is that allright?
        var filters = [
          {
            name: "strikethrough",
            match: /~~[\s\S]*?~~/g,
  ...
  $.GollumPreview.defineRenderer = function(language, options){
    ...
    rendererDefinitions[language].path = function(){
      return ActiveOptions.BaseUrl + '/javascript/editor/parsers/' + language + '/' + this.source + '.js';
    };
